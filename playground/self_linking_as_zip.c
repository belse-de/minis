...
if [ "$0" -nt "$0.out" ]; then
  ...
  gcc $lflags -o "$0.out" "$0.o1" "$0.o2" /usr/lib/x86_64-linux-gnu/libz.a
fi
...
extern uint8_t _binary___oneshot_link_zip_c_start;
extern uint8_t _binary___oneshot_link_zip_c_end  ;
extern uint8_t _binary___oneshot_link_zip_c_size ;

uint8_t* _binary_start_ptr = &_binary___oneshot_link_zip_c_start;
uint8_t* _binary_end_ptr   = &_binary___oneshot_link_zip_c_end;
size_t   _binary_size      = (size_t)&_binary___oneshot_link_zip_c_size;
